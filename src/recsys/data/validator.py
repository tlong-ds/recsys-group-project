...
class InteractionValidator:
    ...
    def validate_semantics(
        ...
        if not allow_duplicates:
            dup_count = df.groupby(self.session_col)[self.item_col].apply(
        ...
        null_counts = df[[self.session_col, self.item_col, self.timestamp_col]].isnull().sum()
        ...
        if valid:
            ...
            logger.warning(f"✗ Semantic validation issues found:")
            for issue in issues:
