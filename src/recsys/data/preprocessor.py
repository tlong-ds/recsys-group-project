...
class SessionPreprocessor:
    ...
    def transform(self, interactions: pd.DataFrame) -> pd.DataFrame:
        ...
        # Diginetica has eventdate (date-only) and timeframe
        # (ordering within session). Keep date as canonical timestamp for split.
        # Use timeframe for in-session ordering.
        ...
        try:
            cleaned[self.item_col] = pd.to_numeric(
                cleaned[self.item_col], errors="coerce"
            )
        except Exception as e:
    ...
    def filter_sessions(
        ...
        filtered = interactions[
            interactions[self.session_col].isin(valid_sessions)
        ].copy()
    ...
    def filter_items(
        ...
        if dropped_min > 0:
            logger.info(
                f"Dropped {dropped_min:,} rows from items "
                f"with < {min_frequency} interactions"
            )
        ...
        #     if dropped_max > 0:
        #         logger.info(
        #             f"Dropped {dropped_max:,} rows from items with "
        #             f"> {max_frequency} interactions"
        #         )
        ...
        stats = {
            ...
            "removed_item_list": list(
                set(interactions[self.item_col]) - set(filtered[self.item_col])
            ),
        }
