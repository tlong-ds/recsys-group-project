...
class SRGNNRecommender(GraphRecommenderBase):
    ...
    def _make_dataset(self, df: pd.DataFrame) -> SessionGraphDataset:
        return SessionGraphDataset(df, variant=self.variant, global_freq=self._global_freq)
    ...
    def _graph_from_sequence(
        ...
        local_freq = self._localise_global_freq(unique_nodes) if self.variant == VARIANT_NGC else None
        adj_np     = build_adjacency_for_variant(
            alias_arr, edge_index, n, self.variant, local_freq,
    ...
    def _tensors_from_graph(
        ...
        adj_np     = build_adjacency_for_variant(
            alias_arr, ei, n, self.variant, local_freq,
    ...
    def _extra_save_state(self, directory: Path) -> None:
        if self.variant == VARIANT_NGC and self._global_freq is not None:
            ...
            serialised = {
                f"{u},{v}": cnt
                for (u, v), cnt in self._global_freq.items()
            }
