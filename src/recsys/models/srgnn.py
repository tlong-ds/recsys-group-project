...
class SRGNNRecommender:
    ...
    def recommend_from_graph(
        ...
        items     = torch.tensor(x_arr,     dtype=torch.long,  device=self.device).unsqueeze(0)
        alias     = torch.tensor(alias_arr, dtype=torch.long,  device=self.device).unsqueeze(0)
        adjacency = torch.tensor(
            _build_adjacency(alias_arr, ei, len(x_arr)),   
            dtype=torch.float32,
            device=self.device,
        ).unsqueeze(0)
